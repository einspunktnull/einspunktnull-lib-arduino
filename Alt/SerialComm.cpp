/*
 SerialComm.h - Arduino Library for SerialComm
 ...
 specific demand to communicate with the SerialComm based Software.
...
void SerialComm::getBuffer(uint8_t buf[]) {
...
void SerialComm::registerFunction(
		void (*userfunction)(uint8_t, uint8_t), uint8_t command) {
	_intFunc[command] = userfunction;
}
void SerialComm::unregisterFunction(uint8_t command) {
	_intFunc[command] = _errorFunc;
}

size_t SerialComm::write(uint8_t b) {
...
int SerialComm::getIntAt(int idx) {
	int values[BYTE_BUFFER_SIZE];
	getIntValues(values);
	return values[idx];
}

void SerialComm::getIntValues(int values[]) {
...
float SerialComm::getFloatAt(int idx) {
	float values[BYTE_BUFFER_SIZE];
	getFloatValues(values);
	return values[idx];
}

void SerialComm::getFloatValues(float values[]) {
	int t = 0; // counter for each char based array
	int pos = 0;
...
double SerialComm::getDoubleAt(int idx) {
	return getFloatAt(idx);
}

void SerialComm::getDoubleValues(float values[]) {
	getFloatValues(values);
}

uint8_t SerialComm::getByte() {
	return _buffer[1];
}

void SerialComm::getString(char string[]) {
 ...
 * PUBLIC SEND
 ******************************************************************************/
void SerialComm::send(char flag) {
	Serial.write(flag);
	Serial.write(BYTE_END);
}

void SerialComm::send(char flag, char val) {
	Serial.write(flag);
	Serial.write(val);
	Serial.write(BYTE_END);
}

void SerialComm::send(char flag, const char str[]) {
	Serial.write(flag);
	Serial.write(str);
	Serial.write(BYTE_END);
}

void SerialComm::send(char flag, uint8_t n) {
	Serial.write(flag);
	Serial.write(n);
	Serial.write(BYTE_END);
}

void SerialComm::send(char flag, int n) {
	Serial.print(flag);
	Serial.print(n);
	Serial.print(BYTE_END);
}
void SerialComm::send(char flag, unsigned int n) {
	Serial.write(flag);
	Serial.write(n);
	Serial.write(BYTE_END);
}
void SerialComm::send(char flag, long n) {
	Serial.write(flag);
	Serial.write(n);
	Serial.write(BYTE_END);
}
void SerialComm::send(char flag, unsigned long n) {
	Serial.write(flag);
	Serial.write(n);
	Serial.write(BYTE_END);
}

void SerialComm::send(char flag, double n) {
	Serial.write(flag);
	Serial.write(n);
	Serial.write(BYTE_END);
}

void SerialComm::sendln(void) {
	char lf = 0x0A;
	Serial.write(lf);
	Serial.write(BYTE_END);
}
